  ...
  const handlePaySupplier = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(`/suppliers/${selectedPurchaseForPayment.supplier_id}/payments`, {
        ...
        purchaseInvoiceNo: selectedPurchaseForPayment.purchase_invoice_no,
        notes: `Payment for Purchase Invoice #${selectedPurchaseForPayment.purchase_invoice_no}. ${paymentNotes}`.trim(),
      });
    ...
    <div className="flex-1 bg-[#F8FAFC] dark:bg-[#090D16] p-8 overflow-y-auto space-y-6 font-sans transition-colors">
                    ...
                    <tr key={p.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 transition">
                      ...
                      <td className="p-4 text-center">
                        {p.receipt_attachment_url ? (
                          <button
                            type="button"
                            onClick={() => setProofPreviewUrl(p.receipt_attachment_url)}
                            className="px-2.5 py-1 rounded-lg bg-blue-50 dark:bg-blue-950/40 hover:bg-blue-100 dark:hover:bg-blue-900/60 text-blue-700 dark:text-blue-300 font-bold text-[10px] inline-flex items-center space-x-1 transition shadow-2xs"
                          >
      ...
      {proofPreviewUrl && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in font-sans">
          <div className="bg-white dark:bg-[#111827] rounded-3xl w-full max-w-2xl max-h-[90vh] p-6 shadow-2xl relative border border-slate-200 dark:border-slate-800 flex flex-col space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center space-x-2">
                <Paperclip className="w-4 h-4 text-blue-500" />
                <h3 className="text-sm font-black text-slate-900 dark:text-white">Vendor Invoice Paper Receipt Proof</h3>
              </div>
              <div className="flex items-center space-x-2">
                <a
                  ...
                  className="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-900 dark:text-white text-xs font-bold transition flex items-center space-x-1"
                >
            ...
            <div className="flex-1 overflow-auto bg-slate-50 dark:bg-slate-900 rounded-2xl p-4 flex items-center justify-center min-h-[300px]">
              {proofPreviewUrl.toLowerCase().endsWith('.pdf') ? (
                <iframe
                  src={proofPreviewUrl}
                  title="PDF Proof"
                  className="w-full h-full min-h-[500px] rounded-xl border border-slate-200 dark:border-slate-800"
                />
              ) : /\.(docx?|xlsx?|csv)$/i.test(proofPreviewUrl) ? (
                <div className="text-center p-8 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm max-w-md w-full space-y-4">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-900 dark:text-white text-sm break-all">
                      {proofPreviewUrl.split('/').pop()}
                    </h4>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                      {/\.(xlsx?|csv)$/i.test(proofPreviewUrl) ? 'Spreadsheet Document' : 'Word Processing Document'}
                    </p>
                  </div>
                  <a
                    href={proofPreviewUrl}
                    download
                    className="inline-flex items-center space-x-2 px-5 py-2.5 bg-slate-950 dark:bg-white hover:bg-slate-850 dark:hover:bg-slate-200 text-white dark:text-slate-950 font-bold text-xs rounded-xl shadow-sm transition"
                  >
